		...
		If Password = PasswordConfirmation Then
			...
			Else
				Message(NStr("ru = 'Администратор с таким именем уже существует.
						|Введите уникальное имя или перейдите к изменению существующего администратора.';SYS=''", "ru"));
			EndIf;
		Else
			Message(NStr("ru = 'Подтверждение пароля не совпадает с паролем';SYS=''", "ru"));
		EndIf;
	EndIf;
EndProcedure
