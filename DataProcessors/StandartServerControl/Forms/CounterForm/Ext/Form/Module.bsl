...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.ItemCounterGroup.ChoiceList.Add(AdministrationResourceConsumptionCounterGroupType.Users, NStr("ru = 'Пользователи'; SYS='Users'", "ru"));
	Items.ItemCounterGroup.ChoiceList.Add(AdministrationResourceConsumptionCounterGroupType.DataSeparation, NStr("ru = 'Разделение данных'; SYS='Data separation'", "ru"));
	// Заполнить список тип фильтра
	Items.ItemCounterFilterType.ChoiceList.Add(AdministrationResourceConsumptionCounterFilterType.All, NStr("ru = 'Все'; SYS='All'", "ru"));
	Items.ItemCounterFilterType.ChoiceList.Add(AdministrationResourceConsumptionCounterFilterType.AllSelected, NStr("ru = 'Все выбранные'; SYS='All selected'", "ru"));
	Items.ItemCounterFilterType.ChoiceList.Add(AdministrationResourceConsumptionCounterFilterType.AllButSelected, NStr("ru = 'Все кроме выбранных'; SYS='All but selected'", "ru"));
	...
	If CheckFilling() Then
		...
		Else
			Message(NStr("ru = 'Счетчик потребления ресурсов с таким именем уже существует. 
				|Введите уникальное имя или перейдите к изменению существующего счетчика.'; SYS=''", "ru"));
		EndIf;
	EndIf;
EndProcedure
...
Procedure ItemAccumulateTimeOnChange(Item)
	If AccumulateTime < 0 Then
		Message(Nstr("ru = 'Период счетчика не может быть отрицательным'; SYS=''", "ru"));
		AccumulateTime = 0;
	EndIf;
EndProcedure
