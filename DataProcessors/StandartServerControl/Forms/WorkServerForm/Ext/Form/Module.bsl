...
Procedure WriteWorkServerAndCloseForm(Command)
	Var ErrorRange;
	ErrorRange = False;
	If SafeMemoryUsageCall < -1 Then
		Message(NStr("ru = 'Поле ""Безопасный расход памяти за один вызов"" 
			|должно быть в диапазоне от -1 до 9223372036854775807.';SYS=''", "ru"));
		ErrorRange = True;
	EndIf;
	If CheckFilling() And Not ErrorRange Then
		...
		Else
			Message(NStr("ru = 'Рабочий сервер уже зарегистрирован в кластере.
					|Введите уникальное имя или перейдите к изменению существующего рабочего сервера.';SYS=''", "ru"));
		EndIf;
	EndIf;
EndProcedure
