...
Procedure ClusterTreeBeforeAddRow(Item, Cancel, Copying, Parent, Group, Parameter)
	...
	If ClusterTree.GetItems().Count() > 0 Then
		...
		Else
			Cancel = True;
			Message(Nstr("ru = 'Невозможно выполнить добавление этого элемента';SYS=''", "ru"));
		EndIf;
	...
	If ClusterTree.GetItems().Count() > 0 Then
		...
		If (CurrentTreeLine.ItemGroup <> True) Then
			...
			ElsIf (Parent.AdministrationItemType = "WorkProcesses") Then
				ParametersStructure = GetParameterStructureWorkProcess(ConnectionSettings, ClusterSettings, CurrentTreeLine.Name);
				OpenFormSettings.Insert("ObjectFieldValues", ParametersStructure);
...
// запрет удаления групп из дерева
&AtClient
Procedure ClusterTreeBeforeDeleteRow(Item, Cancel)
	If ClusterTree.GetItems().Count() > 0 Then
		...
		Cancel = True;
		If (CurrentTreeLine.ItemGroup = True) Then
			Message(Nstr("ru = 'Невозможно выполнить удаление группы элементов'; SYS=''", "ru"));
		Else
			ParametersStructure = New Structure();
			QuestionText = "";
			ParentItemType = CurrentTreeLine.GetParent().AdministrationItemType;
			// удаление сервера 1С:Предприятие
			If ParentItemType = "AdministrationCentralServer" Then
				...
				QuestionText = NStr("ru = 'Вы действительно хотите удалить 
						|соединение с сервером администрирования'; SYS=''", "ru");
			// удаление администрирование кластер
			ElsIf ParentItemType = "Clusters" Then
				QuestionText = NStr("ru = 'Удаление кластера приведет к удалению его настроек и списка зарегистрированных информационных баз.
					|Вы действительно хотите удалить кластер?'; SYS=''", "ru");
			// удаление информационной базы
			ElsIf ParentItemType = "InfoBases" Then
			    QuestionText = NStr("ru = 'Вы действительно хотите удалить информационную базу?'; SYS=''", "ru");
			...
			ElsIf ParentItemType = "WorkServers" Then
			    QuestionText = NStr("ru = 'После удаления рабочего сервера может потребоваться перезапуск клиентских приложений. 
						|Вы действительно хотите удалить рабочий сервер?'; SYS=''", "ru");
			...
			ElsIf ParentItemType = "ClusterAdministrators" Then
				QuestionText = NStr("ru = 'После удаления данные администратора будут потеряны.
							|Вы действительно хотите удалить администратора кластера?'; SYS=''", "ru");
			...
			ElsIf ParentItemType = "SecurityProfiles" Then
				QuestionText = NStr("ru = 'После удаления данные профиля безопасности будут потеряны.
							|Вы действительно хотите удалить профиль безопасности?'; SYS=''", "ru");
			...
			ElsIf ParentItemType = "ResourceConsumptionCounters" Then
				QuestionText = NStr("ru = 'После удаления данные счетчика потребления ресурсов будут потеряны.
							|Вы действительно хотите удалить счетчик?';SYS=''", "ru");
			...
			ElsIf ParentItemType = "ResourceConsumptionLimits" Then
				QuestionText = NStr("ru = 'После удаления данные ограничения потребления ресурсов будут потеряны. 
							|Вы действительно хотите удалить ограничение?'; SYS=''", "ru");
			...
			ElsIf ParentItemType = "ExternalModules" Then
				QuestionText = NStr("ru = 'Удаление внешнего модуля приведет к потере данных о внешнем модуле. ,
						|Вы действительно хотите удалить внешний модуль'; SYS=''", "ru");
				ProfileName = Parent.GetParent().Name;
			// удаление разрешенного приложения
			ElsIf ParentItemType = "AllowedApplications" Then
				QuestionText = NStr("ru = 'Удаление приложения приведет к потере данных о приложении.
						|Вы действительно хотите удалить приложение?'; SYS=''", "ru");
				ProfileName = Parent.GetParent().Name;
			// удаление ресурса интернет
			ElsIf ParentItemType = "InternetResources" Then
				QuestionText = NStr("ru = 'Удаление ресурса интернет приведет к потере данных о ресурсе.
						|Вы действительно хотите удалить ресурс интернет'; SYS=''", "ru");
				ProfileName = Parent.GetParent().Name;
			// удаление администратора кластера
...
Procedure SaveInfobasePassword(val CurrentTreeLine, AdministratorSettings)
	While CurrentTreeLine <> Undefined Do
 		If CurrentTreeLine.GetParent().AdministrationItemType = "InfoBases" Then
			ClusterAndInfobaseUserAuthenticationSettings.Insert(
				CurrentTreeLine.ItemUUID, AdministratorSettings); 
			Break;
		EndIf;
		CurrentTreeLine = CurrentTreeLine.GetParent();
	EndDo;	
EndProcedure
...
Procedure DisplayListOfTreeObjects(TreeLine)
	...
	If Parent <> Undefined Then
		...
		ElsIf CurrentTreeLine.AdministrationItemType = "WorkProcesses" Then
			...
			For each ObjectName In ListOfObjectNames Do
				AddItemToClusterTreeAtClient(ListOfTreeObjects, ObjectName, 14, False);
			EndDo;
		ElsIf CurrentTreeLine.AdministrationItemType = "WorkServers" Then
			ListOfTreeObjects = CurrentTreeLine.GetItems();
			ListOfTreeObjects.Clear();
		...
		ElsIf CurrentTreeLine.AdministrationItemType = "SecurityProfile" Then
			ListOfTreeObjects = CurrentTreeLine.GetItems();
			ListOfTreeObjects.Clear();
			AddItemToClusterTreeAtClient(ListOfTreeObjects, NStr("ru = 'Виртуальные каталоги'; SYS = ''", "ru"), 24, True, "VirtualDirectories");
			AddItemToClusterTreeAtClient(ListOfTreeObjects, NStr("ru = 'Разрешенные COM-классы'; SYS = ''", "ru"), 26, True, "AllowedComClasses");
			AddItemToClusterTreeAtClient(ListOfTreeObjects, NStr("ru = 'Внешние компоненты'; SYS = ''", "ru"), 28, True, "AllowedAddIns");
			AddItemToClusterTreeAtClient(ListOfTreeObjects, NStr("ru = 'Внешние модули'; SYS = ''", "ru"), 30, True, "ExternalModules");
			AddItemToClusterTreeAtClient(ListOfTreeObjects, NStr("ru = 'Разрешенные приложения'; SYS = ''", "ru"), 32, True, "AllowedApplications");
			AddItemToClusterTreeAtClient(ListOfTreeObjects, NStr("ru = 'Ресурсы интернет'; SYS = ''", "ru"), 34, True, "InternetResources");
		ElsIf CurrentTreeLine.AdministrationItemType = "VirtualDirectories" Then
			ProfileName = CurrentTreeLine.GetParent().Name;
			ListOfTreeObjects = CurrentTreeLine.GetItems();
			ListOfTreeObjects.Clear();
		...
		ElsIf CurrentTreeLine.AdministrationItemType = "Clusters" Then
			...
			For each ObjectName In ListOfObjectNames Do
				...
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Информационные базы'; SYS = 'Infobases'", "ru"), 4, True, "InfoBases");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Рабочие серверы'; SYS = 'Work servers'", "ru"), 6, True, "WorkServers");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Администраторы кластера'; SYS = 'Cluster administrators'", "ru"), 9, True, "ClusterAdministrators");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Менеджеры кластера'; SYS = 'Cluster managers'", "ru"), 11, True, "ClusterManagers");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Рабочие процессы'; SYS = 'Work processes'", "ru"), 13, True, "WorkProcesses");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Сеансы'; SYS = 'Sessions'", "ru"), 15, True, "Sessions");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Блокировки'; SYS = 'Locks'", "ru"), 17, True, "Locks");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Соединения'; SYS = 'Connections'", "ru"), 20, True, "Connections");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Профили безопасности'; SYS = 'Security profiles'", "ru"), 22, True, "SecurityProfiles");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Счетчики потребления ресурсов'; SYS = 'Resource consumption counters'", "ru"), 36, True, "ResourceConsumptionCounters");
				AddItemToClusterTreeAtClient(ClusterGroupItems, NStr("ru = 'Ограничения потребления ресурсов'; SYS = 'Resource consumption limits'", "ru"), 38, True, "ResourceConsumptionLimits");
			EndDo;
		ElsIf CurrentTreeLine.AdministrationItemType = "CentralServerAdministrators" Then
			ListOfTreeObjects = CurrentTreeLine.GetItems();
			ListOfTreeObjects.Clear();
...
Procedure FillClusterTreeConnectionList()
	...
	// начинаем заполнять дерево
	// список подключений к разным ras
	ServerGroup.Name = NStr("ru = 'Центральный сервер 1С:Предприятия'; SYS = '1C:Enterprise 8.3 Central Servers'", "ru");
...
Procedure AddServerStructure(ConnectionParameters, TreeLineID)
	...
	ClusterGroup.Name = NStr("ru = 'Кластеры'; SYS = 'Clusters'", "ru");
	...
	For each Cluster In Clusters Do
		...
		ClusterGroupItems = NewCluster.GetItems();
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Информационные базы'; SYS = 'Infobases'", "ru"), 4, True, "InfoBases");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Рабочие серверы'; SYS = 'Work servers'", "ru"), 6, True, "WorkServers");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Администраторы кластера'; SYS = 'Cluster administrators'", "ru"), 9, True, "ClusterAdministrators");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Менеджеры кластера'; SYS = 'Cluster managers'", "ru"), 11, True, "ClusterManagers");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Рабочие процессы'; SYS = 'Work processes'", "ru"), 13, True, "WorkProcesses");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Сеансы'; SYS = 'Sessions'", "ru"), 15, True, "Sessions");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Блокировки'; SYS = 'Locks'", "ru"), 17, True, "Locks");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Соединения'; SYS = 'Connections'", "ru"), 20, True, "Connections");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Профили безопасности'; SYS = 'Security profiles'", "ru"), 22, True, "SecurityProfiles");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Счетчики потребления ресурсов'; SYS = 'Resource consumption counters'", "ru"), 36, True, "ResourceConsumptionCounters");
		AddItemToClusterTree(ClusterGroupItems, NStr("ru = 'Ограничения потребления ресурсов'; SYS = 'Resource consumption limits'", "ru"), 38, True, "ResourceConsumptionLimits");
	EndDo;	
	// администраторы агента
	AgentAdministratorsGroup = ServerGroupContent.Add();
	AgentAdministratorsGroup.Name = NStr("ru = 'Администраторы';SYS = 'Administrators'", "ru");
...
Function GetListOfWorkProcessNames(ConnectionSettings, ClusterSettings)
	...
	For each WorkProcess In Cluster.GetWorkProcesses() Do
		ListOfWorkProcess.Add(WorkProcess.ComputerName);	
	EndDo;
...
&AtServerNoContext
Function GetParameterStructureWorkProcess(ConnectionSettings, ClusterSettings, WorkProcessName)
	...
	For each WorkProcess In WorkProcesses Do
		If WorkProcess.ComputerName = WorkProcessName Then
		...
		If WorkServer.ComputerName = WorkServerName Then
			...
			For each AssignmentRule In AssignmentRules Do
				...
				If IsBlankString(AssignmentRule.ObjectType) Then
					NewTableLine.ObjectType = NStr("ru = 'Для всех';SYS='Any'", "ru");
				ElsIf AssignmentRule.ObjectType = "Connection" Then
					NewTableLine.ObjectType = Nstr("ru = 'Клиентское соединение с информационной базой', SYS = ''", "ru");
				Else
					For each Service In Services Do
				...
				If IsBlankString(NewTableLine.InfoBaseName) Then
					NewTableLine.InfoBaseName = NStr("ru = 'Для всех';SYS=''", "ru");
				EndIf;
...
Procedure ProcessingCloseSaveSettingsForm(Result, AdditionalParameters) Export
	...
	If Result <> Undefined Then
	    ...
	    If AttachFileSystemExtension() Then
			...
			Message(NStr("ru = 'Файл настроек сохранен'; SYS = ''", "ru"));
	    EndIf;
	EndIf;
EndProcedure
...
Procedure UploadFileToServer(Result, InternalAddressServer, FileSettingsPath, AdditionalParameters) Export 
	...
	If LoadSettingsFileResult <> Undefined Then
		...
		Message(Nstr("ru = 'Из файла загружены настройки подключений к серверам администрирования'; SYS = ''", "ru"));
		FillClusterTreeConnectionList();
		RootElements = ClusterTree.GetItems();
		CentralServer = RootElements.Get(0).GetID();
		VertexID = ClusterTree.FindByID(CentralServer);
...
&AtServer
Procedure DeleteConnectionAtServer(ConnectionSettings, ClusterSettings,
								InfoBaseName, ConnectionID)
	Var Cluster, Infobases, Infobase, Connections, Connection;
	Cluster = GetAdministrationCluster(ConnectionSettings, ClusterSettings);
	Infobases = Cluster.GetInfoBases();
	Connections = Cluster.GetConnections();
	For each Connection In Connections Do
...
Procedure DeleteConnection(Command)
	...
	For each HighlightedLine In Items.ItemConnectionsTable.SelectedRows Do
		...
		If InfoBaseParameters = Undefined Or CheckRuleToChangeInfoBase(ConnectionSettings, ClusterSettings, InfoBaseParameters) <> Undefined Then
			OpenAuthenticationInfobaseForm(ConnectionSettings, ClusterSettings, Connection.InfoBaseID, CurrentTreeData);
		EndIf;
		DeleteConnectionAtServer(ConnectionSettings, ClusterSettings,
			InfoBaseParameters, ConnectionID);
	EndDo;
	ConnectionsTable.Clear();
	FillConnectionsTable(ConnectionSettings, ClusterSettings);
EndProcedure
...
Procedure BeforeClose(Cancel, ShutDown, AlertText, StandardProcessing) 
	If ShutDown Or ClosingAllowed = Истина Then
		Cancel = False;
	Else
