...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.FormActionOnExceedingLimits.ChoiceList.Add(
		AdministrationActionOnResourceConsumptionLimitExcess.None, Nstr("ru = 'Нет'; SYS='None'", "ru"));
	Items.FormActionOnExceedingLimits.ChoiceList.Add(
		AdministrationActionOnResourceConsumptionLimitExcess.SetThreadLowPriority, Nstr("ru = 'Установить низкий приоритет потока'; SYS='Set thread low priority'", "ru"));
	Items.FormActionOnExceedingLimits.ChoiceList.Add(
		AdministrationActionOnResourceConsumptionLimitExcess.InterruptCurrentServerCall, Nstr("ru = 'Прервать текущий серверный вызов'; SYS='Interrupt current server call'", "ru"));
 	Items.FormActionOnExceedingLimits.ChoiceList.Add(
		AdministrationActionOnResourceConsumptionLimitExcess.TerminateSession, Nstr("ru = 'Завершить сеанс'; SYS='Terminate session'", "ru"));
	...
	If CheckFilling() Then
		...
		Else
			Message(Nstr("ru = 'Ограничение потребления ресурсов с таким именем уже существует. 
				|Введите уникальное имя или перейдите к изменению существующего ограничения.';SYS=''", "ru"));
		EndIf;
	EndIf;
EndProcedure
