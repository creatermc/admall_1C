	...
	If CheckFilling() Then
		...
		Else
			Message(Nstr("ru = 'Подключение к серверу администрирования с таким именем уже существует. 
				|Введите уникальное имя или перейдите к изменению существующего подключения.';SYS=''", "ru"));
		EndIf;
	EndIf;
EndProcedure
...
Function IsAvaliableAdministrationServer()
	...
	Except
		Message(NStr("ru = 'Неверное имя или пароль администратора центрального сервера';SYS=''", "ru"));
		Return False;	
	EndTry;
	Return True;
EndFunction
