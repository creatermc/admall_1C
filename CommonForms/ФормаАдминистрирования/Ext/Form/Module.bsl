...
Функция ТекстКомандыИзмененияПравНаФайлРезультата() 
	Возврат "echo " + "'"+ ПарольСуперПользователя + "'" + " | sudo -S chmod 667 " + ФайлРезультатСкрипта;
КонецФункции
...
Функция СкриптПолученияИнформацииПоСпискуБазPostgres()
	Возврат "echo " + "'"+ ПарольСуперПользователя + "'" + " | sudo -S -u postgres psql -c " + "'" + "\l" + "'";
КонецФункции
...
Процедура ПолучениеДанныхССервераПослеОбработкиСкрипта(Скрипт1С = Ложь, СкриптDocker = Ложь, 
	СкриптUbuntu = Ложь, ОтображатьТЧ = Ложь, ПрямаяКоманда = Ложь)
