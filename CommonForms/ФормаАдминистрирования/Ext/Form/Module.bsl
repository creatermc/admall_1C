...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтобразитьРезультатВыполнения = Истина;
	ОтобразитьОкноТЗРезультатВыполнения = Истина; 
...
//
// ну немного докрутить. Я думаю, что это нужно не для того, чтоб команды из под рута смотреть... а прочих засранцев :)
...
//
&НаКлиенте
Функция СкриптПолученияИсторииКомандПользователя()
	Возврат "grep -e ""$pattern"" /home/*/.bash_history";
КонецФункции
...
Функция СкриптВыгрузкиCFФайлаБазы1C()
	Возврат "xvfb-run -a /opt/1C/v8.3/i386/1cv8 CONFIG /S" + Символ(34) + idКластераСервера1С.Наименование + "\" + База1С.Наименование + Символ(34) + " /N" + Символ(34) +
	 АдминистраторБазы1С + Символ(34) + " /P" + Символ(34) + ПарольАдминистратораБазы1С + Символ(34) + " /DumpCfg " + 
		?(ЗначениеЗаполнено(ПутьСохраненияФайлаCF), ПутьСохраненияФайлаCF, "~/cfs") + ?(Прав(СокрЛП(ПутьСохраненияФайлаCF), 1) = "/" , "", "/") + 
			База1С.Наименование + "-" + Строка(Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy-HH-mm")) + ".cf";
КонецФункции
...
&НаКлиенте
Процедура ПолучитьСписокБаз1С(Команда)
...
Процедура ПослеЗакрытияВопросаОтключениеПользователей1С(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		СписокАктивныхПользователей.Очистить();
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
...
Процедура ЗакрытиеВсехАктивныхСессий1С();
	...
	ПредСтрокаКоманд = "";
		...
		Для каждого ТекСтрока Из СписокАктивныхПользователей Цикл 
			ПредСтрокаКоманд  = ПредСтрокаКоманд + СкриптУдалениеВсехАктивныхСессии1С(ТекСтрока.idUser) + "; ";
		КонецЦикла;
				  ...
				  КоличСтрокМассива = МассивПоиска[0].Количество() - 1;
					Пока ИндексМассива <= КоличСтрокМассива Цикл 
						...
						Элементы.СписокПользователей1С1.СписокВыбора.Добавить(МассивПоиска[0][ИндексМассива].Реквизит3,
						 	МассивПоиска[1][ИндексМассива].Реквизит3);
								СтрокаidUser = СписокАктивныхПользователей.Добавить();
								СтрокаidUser.idUser = МассивПоиска[0][ИндексМассива].Реквизит3;
						ИндексМассива = ИндексМассива + 1;
					КонецЦикла;
				КонецЕсли;
		КонецЕсли;
...
Процедура ВыгрузитьФайл(Команда)
		...
		ВыполнитьВыгрузкуРезервногоКопирования();

КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьВыгрузкуРезервногоКопирования()

	Если ВариантВыгрузкиРезервногоКопирования = "" Тогда
			Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
			ПоказатьПредупреждение(Оповещение, "Не выбран вариант выгрузки!", 0, "Внимание!"); 
		Возврат;
	КонецЕсли;

	Если ВариантВыгрузкиРезервногоКопирования = "dt" Тогда 
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаОВыгрузкеРезервнойКопии1С", ЭтотОбъект);	
		ПоказатьВопрос(Оповещение, "Перед выгрузкой будут отключены все пользователи!" + Символы.ПС +
		 "Вы уверены что хотите закрыть все сессии пользователей ?", РежимДиалогаВопрос.ДаНет, 0,
		КодВозвратаДиалога.Нет, "Внимание !" );
	ИначеЕсли ВариантВыгрузкиРезервногоКопирования = "cf" Тогда
			Если ОтображатьЗапрос Тогда
				ОчиститьСообщения();
			КонецЕсли;
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаОВыгрузкеРезервнойКопии1С(Результат, Параметры) Экспорт 
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		//Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
		//ПоказатьПредупреждение(Оповещение, "Файл "" базы выгружен в папку "" на сервере "" 
		// !", 0, "Внимание!");
	КонецЕсли;
...
Процедура ВыполнитьВыгрузкуCFФайла()
	...
	//ПолучениеСистемнойИнформации();

	//СохранитьНаСерверИсходникСкрипта();

	//УдалятьBOMСимволыВТекстеФайла();

Сообщить(ТекстПрямойКоманды);
	ПолучениеДанныхССервераПослеОбработкиСкрипта(,,,, Истина); //,, Ложь, Истина);

		//Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
		//ПоказатьПредупреждение(Оповещение, "Файл "" конфигурации базы выгружен в папку "" на сервере "" 
		// !", 0, "Внимание!");
...
&НаСервере
Процедура УдалятьBOMСимволыВТекстеФайла()
...
&НаКлиенте
Процедура ПолучениеСистемнойИнформации()
...
Функция ПолучениеТекущегоИПАдреса() 
	...
	ТекстКомандыПолученияИП = ?(ТунельSSH, "sshpass -p '" + ПарольПользовательСервераЧерезSSH + "' ssh '" + 
		...
		ПолучениеИмениВременногоФайла(?(ТунельSSH, Истина, Ложь));
...
Процедура ПолучениеДанныхССервераПослеОбработкиСкрипта(Скрипт1С = Ложь, СкриптDocker = Ложь, 
							СкриптUbuntu = Ложь, ОтображатьТЧ = Ложь, ПрямаяКоманда = Ложь, ОтображатьЗапрос = Ложь, ВыгружатьВЛогФайл = Истина, СПовышеннымиПравами = Ложь)

	РезультатВыполнения = "";
	АдресДокументаВХранилище = ?(ПрямаяКоманда, ОбработкаКоманды(ОтображатьЗапрос), ОбработкаФайла(ФайлСкрипта,, ВыгружатьВЛогФайл, СПовышеннымиПравами));
...
&НаСервере
Функция ОбработкаФайла(ФайлСкрипта, ч = Ложь, ВыгружатьВЛогФайл, СПовышеннымиПравами)

		Если ВыгружатьВЛогФайл Тогда 
			ПолучениеИмениВременногоФайла(?(ТунельSSH, Истина, Ложь));

				Если СПовышеннымиПравами Тогда 
					//Формируем команды и выполняем с указанным таймаутом в секундах "timeout 10s"
					КомандаЗапускаСкрипта = "echo " + "'" + "JnY7YslKkjn98!k" + "'" + " | sudo -S " +
						 "timeout 10s /bin/bash '" + ФайлСкрипта + "' >> '" + ФайлРезультатСкрипта + "'";	
				Иначе 
			...
			// Обработка результата выполнения	
			Если НЕ КодВозврата = 0 И ОтображатьЗапрос Тогда
		...
		Иначе
				...
				Если СПовышеннымиПравами Тогда 
					//Формируем команды и выполняем с указанным таймаутом в секундах ""
					КомандаЗапускаСкрипта = "echo " + "'" + "JnY7YslKkjn98!k" + "'" + " | sudo -S " +
						 "timeout 30s /bin/bash '" + ФайлСкрипта + "' ";	
				Иначе 
					КомандаЗапускаСкрипта = "timeout 30s /bin/bash '" + ФайлСкрипта + "' ";	
				КонецЕсли;
			...
			КонецЕсли;
					...
					ФайлВоВременномХранилище = ПоместитьВоВременноеХранилище(""); 
...
&НаСервере
Функция ОбработкаКоманды(ОтображатьЗапрос = Ложь)
...
Процедура ПроверкаСуществованияПапки()

КонецПроцедуры

Процедура ПолучитьРазмерСвободногоМестаНаДисках()
...
Процедура ОткрытьВОтдельномОкнеТЧ(Команда)
	ОткрытьФорму("ОбщаяФорма.ФормаРезультатВыполненияТЧ", Новый Структура("РезультатВыполненияТЧ", РезультатВыполненияТЧ),,,,,, 
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
КонецПроцедуры
...
&НаКлиенте
Процедура АдминистраторСервера1С4ОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ПолучениеАдмина1СОбработкаВыбора(ВыбранноеЗначение)
КонецПроцедуры

&НаКлиенте
Процедура АдминистраторБазы1С3ОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ПолучениеАдминаБазы1СОбработкаВыбора(ВыбранноеЗначение);
КонецПроцедуры

&НаКлиенте
Процедура АдминистраторБазы1С2ОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ПолучениеАдминаБазы1СОбработкаВыбора(ВыбранноеЗначение)
КонецПроцедуры

&НаКлиенте
Процедура ОтобразитьРезультатВыполненияПриИзменении(Элемент)

	Если ОтобразитьРезультатВыполнения Тогда 
		ЭтаФорма.Элементы.ГруппаРезультатВыполнения.Видимость = Истина;
	Иначе 
		ЭтаФорма.Элементы.ГруппаРезультатВыполнения.Видимость = Ложь;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ОтобразитьОкноТЗРезультатВыполненияПриИзменении(Элемент)
	
	Если ОтобразитьОкноТЗРезультатВыполнения Тогда 
		ЭтаФорма.Элементы.ГруппаРезультатВыполненияТЧ.Видимость = Истина;
	Иначе 
		ЭтаФорма.Элементы.ГруппаРезультатВыполненияТЧ.Видимость = Ложь;
	КонецЕсли;
