...
//  xvfb-run  Xvfb (X virtual framebuffer) — виртуальный X-сервер, который для вывода использует не видеокарту, а оперативную память
&НаСервере
...
Процедура ПолучитьСписокКонтейнеровDocker1(Команда)
	...
	ПолучениеИмениВременногоФайла();
		...
		РезультатВыполнения = ТекстКомандыДляВыполнения;
	СохранитьНаСерверИсходникСкрипта();
	ПолучениеДанныхССервераПослеОбработкиСкрипта(,Истина);
...
Процедура ПолучениеДанныхССервераПослеОбработкиСкрипта(Скрипт1С = Ложь, СкриптDocker = Ложь, 
	...
	Иначе
			РезультатВыполнения = "Не получены данные после обработки !";
		Возврат;
	КонецЕсли;
...
Функция ОбработкаФайла(ФайлСкрипта)
		...
		// Обработка результата выполнения	
		Если НЕ КодВозврата = 0 Тогда
			РезультатВыполнения = РезультатВыполнения + Символы.ПС + " Исключение! Не удалось выполнить команду. " + Символы.ПС +  КомандаЗапускаСкрипта
...
//http://trainingweb.ru/page/line-by-line-pull-strings-file-using-while-cycle-bash

//Bash: Построчное Чтение Файла — While Read Line Цикл
//https://www.shellhacks.com/ru/bash-read-file-line-by-line-while-read-line-loop/
...
//Справочник
//На страницах нашего сайта собран основной теоретический материал по математике, геометрии, физике и другим техническим предметам. У нас подробно разобраны основные формулы, таблицы и теоремы из школьного и ВУЗовского курсов. Каждая теоретическая статья содержит примеры решения задач по теме, чтобы вы сразу могли увидеть, как применяется данный теоретический материал на практике!
