	...
	Если ОтображатьТЧ Тогда 
		Если Скрипт1С Тогда 
			...
			Для НомерСтроки = 1 по ЗагружаемыйФайл.КоличествоСтрок() Цикл
				...
				СтрокаТЧ.Реквизит1 = ?(ЗначениеЗаполнено(СокрЛП(СтрПолучитьСтроку(ТекущСтрока, 1))), СокрЛП(СтрПолучитьСтроку(ТекущСтрока, 1)), "-");
				СтрокаТЧ.Реквизит2 = Разделитель;
				СтрокаТЧ.Реквизит3 = СокрЛП(СтрПолучитьСтроку(ТекущСтрока, 2));

				//ЭтаФорма.Элементы.РезультатВыполненияТЧРеквизит1.ЦветФона = Новый Цвет(0, 0, 255);
...
//https://andreyex.ru/operacionnaya-sistema-linux/kak-chitat-fajl-postrochno-v-bash/
