...
Процедура ВыполнитьСформированныйТекстКоманды()
	...
	ПолучениеДанныхССервераПослеОбработкиКоманды();
...
Процедура ПолучениеДанныхССервераПослеОбработкиСкрипта(Скрипт1С = Ложь, СкриптDocker = Ложь, 
	СкриптUbuntu = Ложь, ОтображатьТЧ = Ложь)
...
&НаСервере
Процедура ПолучениеДанныхССервераПослеОбработкиКоманды(Скрипт1С = Ложь, СкриптDocker = Ложь, 
	СкриптUbuntu = Ложь, ОтображатьТЧ = Ложь)

	РезультатВыполнения = "";
	АдресДокументаВХранилище = ОбработкаКоманды();
...
Функция ОбработкаКоманды()
		...
		ФайлРезультатСкрипта = КаталогВременныхФайлов() +  ИмяФайла + "." + "log";
		 ...
		 //Формируем команды и выполняем с указанным таймаутом в секундах
			 ТекстКоманды ="timeout " + "10s '" + ТекстПрямойКоманды 
				+  "' >> '" + ФайлРезультатСкрипта + "'";	
		...
		Если НЕ КодВозврата = 0 Тогда
			РезультатВыполнения = РезультатВыполнения + Символы.ПС + " Исключение! Не удалось выполнить команду. " + 
				Символы.ПС +  ТекстКоманды +  Символы.ПС  + "Код возврата: " + КодВозврата + Символы.ПС + ТекстКомандыДляВыполнения;
		КонецЕсли;
