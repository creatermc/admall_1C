...
Процедура ВыгрузитьФайл(Команда)
		...
		//ВариантВыгрузкиРезервногоКопирования;
...
Процедура ПолучениеДанныхССервераПослеОбработкиСкрипта(Скрипт1С = Ложь, СкриптDocker = Ложь, 
	...
	РезультатВыполнения = "";
	АдресДокументаВХранилище = ?(ПрямаяКоманда, ОбработкаКоманды(ОтображатьЗапрос), ОбработкаФайла(ФайлСкрипта, ОтображатьЗапрос));
...
&НаСервере
Функция ОбработкаФайла(ФайлСкрипта, ОтображатьЗапрос = Ложь)
