...
// Получить список баз 1С
	//Если ВаринатВзаимодействия1С = "Список баз" Тогда 
...
// Получить список активных пользователей базы 1С
	//Если ВаринатВзаимодействия1С = "Список активных пользователей" Тогда
	//		Если ТунельSSH Тогда  
	//			Если Не ЗначениеЗаполнено(АдминистраторСервера1С) Или Не ЗначениеЗаполнено(ПарольАдминистратораСервера1С) Или
	//						Не ЗначениеЗаполнено(АдресСервераЧерезSSH) Или Не ЗначениеЗаполнено(ПользовательСервераЧерезSSH) Или
	//						Не ЗначениеЗаполнено(ПарольПользовательСервераЧерезSSH) Или Не ЗначениеЗаполнено(База1С) Тогда
...
// Отключение пользователей из базы 1С
	//Если ВаринатВзаимодействия1С = "Отключение пользователей из базы" Тогда
	//	Если ТунельSSH Тогда  
	//		Если 	Не ЗначениеЗаполнено(АдминистраторСервера1С) Или Не ЗначениеЗаполнено(ПарольАдминистратораСервера1С) Или
	//			Не ЗначениеЗаполнено(АдресСервераЧерезSSH) Или Не ЗначениеЗаполнено(ПользовательСервераЧерезSSH) Или
	//			Не ЗначениеЗаполнено(ПарольПользовательСервераЧерезSSH) Или Не ЗначениеЗаполнено(База1С) Или 
	//			Не ЗначениеЗаполнено(СписокПользователей1С)  Тогда
	//				Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
	//					ПоказатьПредупреждение(Оповещение, "Не заполнены основные поля !", 0, "Внимание!");
	//			Возврат;
	//		КонецЕсли;
	//	Иначе 
	//		Если 	Не ЗначениеЗаполнено(АдминистраторСервера1С) Или Не ЗначениеЗаполнено(ПарольАдминистратораСервера1С) Или
	//			Не ЗначениеЗаполнено(База1С) Или Не ЗначениеЗаполнено(СписокПользователей1С)  Тогда
	//				Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
	//					ПоказатьПредупреждение(Оповещение, "Не заполнены основные поля !", 0, "Внимание!");
	//			Возврат;
	//		КонецЕсли;
	...
	//	Если Элементы.СписокПользователей1С.СписокВыбора.НайтиПоЗначению(СписокПользователей1С) = Неопределено Тогда
	//		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
	//		ПоказатьПредупреждение(Оповещение, "Текущий пользователь 1С " + Символ(34) + СписокПользователей1С + Символ(34) +
	//		 " не обнаружена", 0, "Внимание!");
	//		Возврат;
	//	КонецЕсли;

	//    Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаОтключениеПользователей1С", ЭтотОбъект);	
	//     ПоказатьВопрос(Оповещение, "Вы уверены что хотите закрыть текущую сессию " + Символ(34) +
	//		 Элементы.СписокПользователей1С.СписокВыбора.НайтиПоЗначению(СписокПользователей1С).Представление  + Символ(34) + "?",
	//        	РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет, "Внимание!" );
	//КонецЕсли;
//
...
// Проверить заполенеи срипта 
//СкриптПроверкиСуществованияПапкиНаСеврере(ЗаписьТекста, ФайлСкрипта)
	...
	Если ВаринатВзаимодействияUbuntu = "Запущенные сервисы" Тогда
			Если ТунельSSH Тогда   
				Если 	Не ЗначениеЗаполнено(АдресСервераЧерезSSH) Или Не ЗначениеЗаполнено(ПользовательСервераЧерезSSH) Или
					Не ЗначениеЗаполнено(ПарольПользовательСервераЧерезSSH) Тогда
						Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
				...
				КонецЕсли;
			КонецЕсли;		
		ПолучитьСписокВсехЗапущенныхСервисовUbuntu();
	КонецЕсли;
...
&НаКлиенте
Процедура ПолучитьСписокВсехЗапущенныхСервисовUbuntu();
...
Сообщить(Параметры.ТекстовыйДокумент.ПолучитьТекст());
...
Процедура ТунельSSHПриИзменении(Элемент)
	...
	Если ТунельSSH Тогда 
		Элементы.АдресСервераЧерезSSH.Доступность = Истина;
		Элементы.ПользовательСервераЧерезSSH.Доступность = Истина;
		Элементы.ПарольПользовательСервераЧерезSSH.Доступность = Истина;
	Иначе 
		Элементы.АдресСервераЧерезSSH.Доступность = Ложь;
		Элементы.ПользовательСервераЧерезSSH.Доступность = Ложь;
		Элементы.ПарольПользовательСервераЧерезSSH.Доступность = Ложь;
		АдресСервераЧерезSSH = "";
...
Функция СкриптПолученияСпискаКонтейнеровDocker()
	Возврат "docker ps >> " + "'" + ФайлРезультатСкрипта + "'";
КонецФункции
...
#Область СписокСкриптовUbuntu

&НаКлиенте
Функция СкриптПолученияВсехЗапущенныхСервисовUbuntu()
	Возврат "systemctl list-units --type=service";
КонецФункции
...
Функция СкриптПерезагрузкиСервера1С()
	Возврат "echo " + "'"+ ПарольСуперПользователя + "'"+ " | sudo -S service srv1cv83 restart";
...
Функция СкриптОстановкиСервера1С()
	Возврат "echo " + "'"+ ПарольСуперПользователя + "'"+ " | sudo -S service srv1cv83 stop";
КонецФункции
...
// Скрипт удаления активной сессии  на сервере в кластере сервера 1С
//
&НаСервере
...
// Скрипт удаления всех активных сессии  на сервере в кластере сервера 1С
//
&НаСервере
Функция СкриптУдалениеВсехАктивныхСессии1С(IDПользователя)
	Возврат "/opt/1C/v8.3/i386/rac session --cluster=" + idКластераСервера1С.IDСервера1С + " --cluster-user=" + 
		АдминистраторСервера1С + " --cluster-pwd=" +  ПарольАдминистратораСервера1С + " terminate --session=" + IDПользователя;
КонецФункции
...
Сообщить("24: "  + ФайлСкрипта);
Сообщить("24: "  + ФайлРезультатСкрипта);
...
&НаКлиенте
Процедура ВариантSSHПриИзменении(Элемент)

	Если ВариантSSH = "Через SSH" Тогда
		ТунельSSH = Истина;
		Элементы.ГруппаРеквизитыПодключения.Доступность = Истина;

				// ДобавляемыйСписок = Новый Массив;
				// ДобавляемыйСписок.Добавить("провка связи");   
				// ДобавляемыйСписок.Добавить("скорость соединения");   
				// Элементы.ВариантSSH.СписокВыбора.ЗагрузитьЗначения(ДобавляемыйСписок); 

			Если  Элементы.ВариантSSH.СписокВыбора.Количество() = 2 Тогда 
				Элементы.ВариантSSH.СписокВыбора.Добавить("проверка связи", "проверка связи");
				Элементы.ВариантSSH.СписокВыбора.Добавить("скорость соединения", "скорость соединения");
			КонецЕсли;
			//Элементы.ВыполнитьСформированныйСкрипт.Доступность = Ложь;

	ИначеЕсли ВариантSSH = "Локально" Тогда
		ТунельSSH = Ложь; 
		Элементы.ГруппаРеквизитыПодключения.Доступность = Ложь;
		АдресСервераЧерезSSH = "";
			ПользовательСервераЧерезSSH = "";
			ПарольПользовательСервераЧерезSSH = "";
				Пока Элементы.ВариантSSH.СписокВыбора.Количество() > 2 Цикл
						Элементы.ВариантSSH.СписокВыбора.Удалить(Элементы.ВариантSSH.СписокВыбора.Количество() - 1);
				КонецЦикла;

			//Элементы.ВыполнитьСформированныйСкрипт.Доступность = Ложь;
	ИначеЕсли ВариантSSH = "проверка связи" Тогда
			//Элементы.ВыполнитьСформированныйСкрипт.Доступность = Истина;			
			Элементы.ГруппаРеквизитыПодключения.Доступность = Истина;
			ИзменитьСодержаниеПодсказкиКоманды("проверка связи");

	ИначеЕсли ВариантSSH = "скорость соединения" Тогда
			//Элементы.ВыполнитьСформированныйСкрипт.Доступность = Истина;
			Элементы.ГруппаРеквизитыПодключения.Доступность = Истина;
			ИзменитьСодержаниеПодсказкиКоманды("скорость соединения");
	КонецЕсли;
		...
		Поискname = РезультатВыполненияТЧ.НайтиСтроки(Новый Структура("Реквизит1", "name"));
			...
			Для Каждого Строкаотбора Из Поискinfobaser Цикл 
				...
				Если Не ПроверкаНаИмеющуюсяЗаписьВСправочникеБаза1С(Поискinfobaser[ИндексМассива].Реквизит3) Тогда
						...
						ОбъектСправочникаБаза1С.Сервер1С = ТекущСервер1С.Ссылка ;
					Попытка
...
Процедура ПолучитьСписокКластеров1С(Команда)
    ...
    Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект);
     ПоказатьВопрос(Оповещение, "Перед обновлением старый список будет удален." + Символы.ПС + "Выполнить обновление ?",
        РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Нет, "Внимание !");    
...
Процедура ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
    ...
    Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Элементы.idКластераСервера1С.СписокВыбора.Очистить();
		...
		Элементы.idКластераСервера1С7.СписокВыбора.Очистить();
			...
			Элементы.idКластераСервера1С.ОбновитьТекстРедактирования();
...
Процедура ОчисткаСправочникаСервер1С()
	...
	Пока ВыборкаБаза1С.Следующий() Цикл 
		...
		СпрБаза1С.Сервер1С = Справочники.Сервер1С.ПустаяСсылка();
			Попытка
				СпрБаза1С.Записать();
			Исключение
				Сообщить(ОписаниеОшибки());
...
Процедура ОбработкаСкриптПерезагрузкиСервера1С(ФайлСкрипта)
		...
		Если НЕ КодВозврата = 0 Тогда
			Сообщить("Исключение! Не удалось выполнить команду. " + Символы.ПС +  КомандаЗапускаСкрипта +  Символы.ПС  + "Код возврата: " + КодВозврата);
		КонецЕсли;
...
Процедура ОбработкаСкриптаОстановкиСервера1С(ФайлСкрипта, ПарольСуперПользователя)
		...
		//Сообщить(КомандаЗапускаСкрипта);
...
Процедура ОтключитьВсехПользователей(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаОтключениеПользователей1С", ЭтотОбъект);	
	ПоказатьВопрос(Оповещение, "Вы уверены что хотите закрыть все сессии пользователй ?", РежимДиалогаВопрос.ДаНет, 0,
...
Процедура ВыполнитьОтключениеВсехПользователей1С()
		...
		ТекстКомандыДляВыполнения = ?(ТунельSSH, "sshpass -p '" + ПарольПользовательСервераЧерезSSH + "' ssh '" + 
			ПользовательСервераЧерезSSH + "'@'" + АдресСервераЧерезSSH + "' " + СкриптПолучениеДанныхIDАктивныхПользователяхБазы1С() + " ", 
				СкриптПолучениеДанныхIDАктивныхПользователяхБазы1С()); 

	СохранитьНаСерверИсходникСкрипта();
	ПолучениеДанныхССервераПослеОбработкиСкрипта(Истина,,, Ложь);

		//// Конвертируем файл скрипта, удаляя все следы "Windows"
		//ПреобразоватьФайлТекстаБезBOMСимвола(ФайлСкрипта);

			//Если РезультатВыполненияТЧ.Количество() = 0 Тогда 
			//	Сообщить("Активных сессий в базе " + Символ(34) + База1С.Наименование + Символ(34) +
...
Процедура ЗакрытиеВсехАктивныхСессий1С();
	...
	КоличествоСтрокРезультатВыполнения = СтрЧислоСтрок(ПромежуточныеПолученныеДанные);
		Для Индекс = 1 по КоличествоСтрокРезультатВыполнения Цикл
			ПредСтрокаКоманд  = ПредСтрокаКоманд + СкриптУдалениеВсехАктивныхСессии1С(СтрПолучитьСтроку(ПромежуточныеПолученныеДанные ,Индекс))
			+ ?(Индекс < КоличествоСтрокРезультатВыполнения,"; ","");
		КонецЦикла;
	ТекстКомандыДляВыполнения = ?(ТунельSSH,  "sshpass -p '" + ПарольПользовательСервераЧерезSSH + "' ssh '" + 
	ПользовательСервераЧерезSSH + "'@'" + АдресСервераЧерезSSH + "' " + "'"+ ПредСтрокаКоманд + "'", "'" + ПредСтрокаКоманд + "'");
...
Процедура ОтключитьПользователяБазы1С(Команда)
	...
	ПоказатьВопрос(Оповещение, "Вы уверены что хотите закрыть сессию пользователя?", РежимДиалогаВопрос.ДаНет, 0,
	КодВозвратаДиалога.Нет, "Внимание !" );
...
#Область ПолучитьСписокАктивныхПользователей
...
Процедура ЗаполнитьСписокАктивныхПользоваталей1С()
    ...
    Строки = СтрЗаменить(ПолучитьСписокАктивныхПользователейБазы1СSSH(), ":", Символы.ПС);

	Для Индекс = 1 По СтрЧислоСтрок(Строки) Цикл
		ТекСтрока  = СтрПолучитьСтроку(Строки, Индекс);
			Если 	СокрЛП(ТекСтрока) = "session" Тогда
				Элементы.СписокПользователей1С.СписокВыбора.Добавить(СокрЛП(СтрПолучитьСтроку(Строки, Индекс+1)), 
					СокрЛП(СтрПолучитьСтроку(Строки, Индекс+11)));
				Элементы.СписокПользователей1С1.СписокВыбора.Добавить(СокрЛП(СтрПолучитьСтроку(Строки, Индекс+1)), 
					СокрЛП(СтрПолучитьСтроку(Строки, Индекс+11)));
			КонецЕсли; 
	КонецЦикла;

		Если Элементы.СписокПользователей1С.СписокВыбора.Количество() > 0 Тогда
			РезультатВыполнения = "Загрузка данных по активным сеансам пользователей 1С произведена" + Символы.ПС + 
				" Заполните поле " + Символ(34) + "список активных пользователей" + Символ(34);
		Иначе 
			Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
			ПоказатьПредупреждение(Оповещение, "После загрузки, данных по активным пользователям 1С не обраружено !", 0, "Информация!");
		КонецЕсли;
...
Функция ПолучитьСписокАктивныхПользователейБазы1СSSH()
	...
	КонецЕсли;
		...
		ЗаписьТекста.ЗаписатьСтроку("");
		ЗаписьТекста.ЗаписатьСтроку("");
		ЗаписьТекста.ЗаписатьСтроку("# end script:");
		ЗаписьТекста.Закрыть();
...
// то есть код одновременно и Клиент и МобильноеПриложениеКлиент.
   #Если МобильноеПриложениеКлиент Тогда
		Сообщить("МобильноеПриложениеКлиент 2");
   #КонецЕсли
...
&НаКлиенте
Процедура ПолучитьСписокКонтейнеровDocker1(Команда)
...
Сообщить(ТекстКомандыДляВыполнения);
	СохранитьНаСерверИсходникСкрипта();
	ПолучениеДанныхССервераПослеОбработкиСкрипта(,Истина);
...
#Область ПолучитьСтатусРегламентныхЗаданий
...
Процедура СохранитьНаСерверИсходникСкрипта()
	...
	СистемнаяИнфо = Новый СистемнаяИнформация;
		ИнформацияЗаполнения = "ОС: " + СистемнаяИнфо.ТипПлатформы + "; IP Adress: " + "; Процессор: " 
			+ СистемнаяИнфо.Процессор + "; Оперативная память: " + СистемнаяИнфо.ОперативнаяПамять + ";"; 
...
Процедура ПолучениеДанныхССервераПослеОбработкиСкрипта(Скрипт1С = Ложь, СкриптDocker = Ложь, 
	СкриптUbuntu = Ложь, ОтображатьТЧ = Истина)
	
	АдресДокументаВХранилище = ОбработкаФайла(ФайлСкрипта);
	...
	Иначе
			//Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
			//ПоказатьПредупреждение(Оповещение, "После загрузки, данных не обраружено !", 0, "Информация!"); 
			ВызватьИсключение "Не получены данные после обработки !";
		Возврат;
	КонецЕсли;
	...
	Если 	Не ЗначениеЗаполнено(ПолученныеДанныеСтрока) Тогда
			//Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
			//ПоказатьПредупреждение(Оповещение, "После загрузки, данных не обраружено !", 0, "Информация!"); 
			//Сообщить("После загрузки, данных не обраружено !");
	КонецЕсли;
	...
	Если ОтображатьТЧ Тогда 
		...
		КонецЕсли;
	Иначе 
		ПромежуточныеПолученныеДанные = ПолученныеДанныеСтрока;
	КонецЕсли;
...
Функция ОбработкаФайла(ФайлСкрипта)
		...
		// Обработка результата выполнения	
		Если НЕ КодВозврата = 0 Тогда
			Сообщить("Исключение! Не удалось выполнить команду. " + Символы.ПС +  КомандаЗапускаСкрипта
				 +  Символы.ПС  + "Код возврата: " + КодВозврата + Символы.ПС + ТекстКомандыДляВыполнения);
		КонецЕсли;
			...
			УдалитьФайлы(ФайлСкрипта);
				...
				//Сообщить(ФайлРезультатСкрипта);
				//Сообщить(ФайлСкрипта);
...
&НаСервере
Процедура ПолучениеИмениВременногоФайла()
	ИмяФайла = Строка(Новый УникальныйИдентификатор);
	ФайлРезультатСкрипта = КаталогВременныхФайлов() +  ИмяФайла + "." + "log";
КонецПроцедуры
